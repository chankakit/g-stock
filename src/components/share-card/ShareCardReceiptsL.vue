...
function drawHeaderFooter(ctx) {
  ctx.fillStyle = 'rgba(64,71,74,0.5)'
  ...
  titleImg.src = '/imgs/share-card/receipts/receipts-title-d.svg'
  titleImg.onload = () => {
...
function drawStockTitle(ctx) {
  ...
  ctx.fillStyle = 'rgba(64,71,74,0.6)'
...
function drawDataBox(x, y, boxWidth, boxHeight, title, value, ctx) {
  ctx.fillStyle = 'rgba(64,71,74,0.8)'
  ctx.font = `500 12px ${fontFamily}`
...
function drawStockPrice(ctx) {
  ctx.strokeStyle = 'rgba(64,71,74,0.15)'
  let boxYPos = 208
  let boxSize = {
...
function drawRpsBox(x, y, boxWidth, boxHeight, title, value, ctx) {
  const titleColor = 'rgba(37, 41, 43, 0.6)'
  const valueColor = '#40474A'
